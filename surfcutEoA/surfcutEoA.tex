...
An embedding is cellular if its faces are homeomorphic to the plane.
...
In particular, planar minimum cut is dual to shortest generating cycle in an annulus \cite{is-mfpn-79, r-mstcp-83}.

Two cycles are homotopic if one can be continuously deformed to the other.  
Two even subgraphs are homologous if their symmetric difference is the boundary of a subset of the surface.
The edges of a cellularly embedded graph can be decomposed into three subsets $(T, C, X)$ where $T$ is a spanning tree of $G$, $C^*$ is a spanning tree of $G^*$ and $X$ is a set of $2g$ edges.  
Adding each member of $X$ to $T$ one obtains a unique cycle.  
