...
%\anote{(do we need to prove $x(H\oplus H') = x(H) \oplus x(H')$? this seems more general than the fact that the crossing number is independent of the cycle decomposition that one of the reviewers asked to prove?)}
\end{proof}
...
As mentioned in Section~\ref{sec:forest-cotree}, we cannot consistently define when a shortest path crosses an even subgraph.  We avoid this issue by considering crossings between an entire arc in the greedy system and an arbitrary ribbon perturbation of an arbitrary cycle decomposition of the even subgraph.  We emphasize that different cycle decompositions and different ribbon decompositions of the same even subgraph can have different numbers of crossings with the same arc.  The following crossing bound applies to \emph{every} ribbon decomposition of \emph{every} cycle decomposition.
...
There is a $\Z_2$-minimal even subgraph~$H'$ homologous to $H$ such that for any index $i$, and for any ribbon perturbation $\set{\tilde{a}_i, \tilde\gamma_1, \dots, \tilde\gamma_r}$ of any cycle decomposition $\set{\gamma_1, \dots, \gamma_r}$ of $H'$, the total number of crossings between the perturbed arc $\tilde{a}_i$ and the perturbed cycles $\tilde\gamma_1,  \dots, \tilde\gamma_r$ is at most $12g+4b-5$.
\end{lemma}
...
\medskip
Now we begin our main argument.  Fix an are $a_i$ in our greedy system of arcs.
Let $H'$ be an arbitrary $\Z_2$-minimal even subgraph homologous to $H$ under the new edge weights.
Let $\gamma_1, \gamma_2, \dots, \gamma_r$ be an arbitrary cycle decomposition of $H’$, and let $\set{\tilde{a}_i, \tilde\gamma_1, \dots\tilde\gamma_r}$ be an arbitrary ribbon perturbation of $a_i$ and the cycles $\gamma_j$.  Without loss of generality, we can safely assume that $\tilde{a}_i = a_i$.

Recall from Section \ref{sec:forest-cotree} that $a_i$ is the concatenation of a shortest path $\sigma_i$ in the forest $F$, a single leftover edge $e_i\in L$, and the reversal of a shortest path $\tau_i$ in $F$.  For each index $j$, we define a \EMPH{detour} of $\tilde\gamma_j$ from $\sigma_i$ as a subpath of $\tilde\gamma_j$ that has endpoints on $\sigma_i$ but is otherwise disjoint from $\sigma_i$; we define detours from $\tau_i$ similarly.  The number of crossings between $\tilde\gamma_j$ and $a_i$ is at most one more than the number of detours from $\sigma_i$ and $\tau_i$.
