...
Recall from Section \ref{sec:forest-cotree} that each arc $a_i$ is the concatenation of a shortest path $\sigma_i$ in the forest $F$, a single leftover edge $e_i\in L$, and the reversal of a shortest path $\tau_i$ in $F$.  Let $\tilde\sigma_i$ and $\tilde\tau_i$ denote the components of $\tilde{a}_i \setminus e_i^\square$ containing~$\sigma_i$ and $\tau_i$, respectively.  Both $\tilde\sigma_i$ and $\tilde\tau_i$ are paths within the ribbon decomposition $\Sigma^\square$ with endpoints on the boundaries of vertex regions.  Every point of intersection between $\tilde\gamma_j$ and $\tilde{a}_i$ is either a point in $\tilde\gamma_j\cap\tilde\sigma_i$ or a point in $\tilde\gamma_j\cap\tilde\tau_i$.
...
%
For the same of argument, suppose some loop $\ell \in \mathcal{L}$ is contractible.  Without loss of generality, this loop is the contraction of some subpath $\tilde\gamma_1(p,q)$ of cycle $\tilde\gamma_1$; the endpoints $p$ and $q$ of this subpath lie on $\tilde\sigma_i$.  Let $u$ and $v$ be the vertices of $G$ such that $p\in u^\square$ and $q\in v^\square$.  Fix points $p^\flat \in u^\square\cap \tilde\gamma_1$ and $q^\sharp \in v^\square\cap\tilde\gamma_j$ such that the subpath $\tilde\pi = \tilde\gamma_1(p^\flat, q^\sharp)$ strictly contains $\tilde\gamma_1(p,q)$ but has no more intersections with any arc $\tilde{a}_i$.  Let $\pi$ be the path from $u$ to $v$ in $G$ determined by the sequence of vertex regions and edge ribbons traversed by $\tilde\pi$.

Now let $\tilde\rho$ be a path from $q^\sharp$ to $p^\flat$ that closely parallels $\tilde\sigma(q,p)$; specifically, $\tilde\rho$ traverses the same sequence of vertex regions and edge ribbons as $\tilde\sigma(q,p)$, without crossing any perturbed arc $\tilde{a}_i$.  Let $\rho$ be the path from $v$ to $u$ obtained by pulling $\tilde\rho$ back to $G$.  Because $\rho$ is a subpath of the shortest path $\sigma$, it is actually a \emph{shortest} path from $v$ to $u$.
...
The cycle $\tilde\gamma_1’ = \tilde\gamma_1 \oplus \tilde\delta$ intersects $\sigma$ strictly fewer times than $\tilde\gamma_1$, and does not cross any arc $\tilde{a}_i$ more times than~$\tilde\gamma_1$.  Thus, the cycles $\tilde\gamma_1’, \tilde\gamma_2, \dots, \tilde\gamma_\beta$ cross the perturbed arcs less than $X$ times.  The cycle $\tilde\gamma_1’$ is not necessarily simple or disjoint from the other cycles $\tilde\gamma_j$ in the perturbed cycle decomposition.  By replacing small neighborhoods of each self-intersection point of $\tilde\gamma_1’$ and of each intersection point $\tilde\gamma_1’ \cap \tilde\gamma_j$ with a pair of disjoint paths, we obtain a perturbed cycle decomposition of $H^-$ with fewer than $X$ crossings.  But this contradicts our definition of $X$; we conclude that no loop in $\mathcal{L}$ is contractible.

%
\medskip
\paragraph{\textbf{No homotopic loops}}  \jnote{This still needs to be updated.}
%
...
%
%
%%For each index $j$, we define a \EMPH{detour} of $\tilde\gamma_j$ from $\sigma_i$ as a subpath of $\tilde\gamma_j$ that has endpoints on $\sigma_i$ but is otherwise disjoint from $\sigma_i$; we define detours from $\tau_i$ similarly.  The number of crossings between $\tilde\gamma_j$ and $a_i$ is at most one more than the number of detours from $\sigma_i$ and $\tau_i$.
%
%%\jnote{I don’t think the previous sentence is correct. The perturbed cycle $\tilde\gamma_j$ could pass through the vertex regions at the endpoints of $e_i$ multiple times, crossing $a_i$ each time but never intersecting $\sigma_i$ or $\tau_i$, and therefore without creating detours.  There’s an unavoidable tension between working directly in the graph (where we can do shortest-path things) and working in the ribbon perturbation (where crossings are well-defined).  I’m tempted to modify the argument as follows:}
%
%\modified{
...
%We define a \EMPH{detour} of any cycle \(\gamma_j \in \Set{\gamma_1, \dots, \gamma_r}\) from
...
%%We now argue that the total number of detours from \(\sigma\) is at most $6g+2b-3$.
