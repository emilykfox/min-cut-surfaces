...
\begin{theorem}
\label{thm:min-cycle}
Let $G$ be a graph with positive edge weights, cellularly embedded on a surface~$\Sigma$ with first Betti number $\beta$, and let~$\cycle$ be a cycle in~$G$ with $k$ edges.  A shortest cycle in~$\Sigma$ that is $\Z_2$-homologous with~$\cycle$ can be computed in $O(\beta k + 8^\beta \beta^3\, n\log n)$ time.
\end{theorem}
\rnote{
...
\begin{theorem}
\label{thm:min-even}
Let $G$ be an undirected graph with positive edge weights, cellularly embedded on a
...
%and $b$ boundary components,
and let $s$ and~$t$ be vertices of $G$.  We can compute the minimum-weight $(s,t)$-cut
