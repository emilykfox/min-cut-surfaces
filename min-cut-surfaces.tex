...
Lemma~\ref{lem:disk} implies that slicing the surface $\Sigma^\square$ along the perturbed arcs $\tilde{A}$ yields a topological disk $D = \Sigma^\square\snip\tilde{A}$.  The perturbed cycles in $\tilde{H}$ appear as disjoint cycles and boundary-to-boundary paths in $D$, and each perturbed arc $\tilde{a}_i\in \tilde{A}$ appears as a pair of interior-disjoint paths on the boundary of $D$.

Now color the components of $D \snip \tilde{H}$ red and blue, so that components that are adjacent in $D$ have opposite colors.  Because $\tilde{H}$ intersects each arc $\tilde{a}_i$ an even number of times, every component of $D \snip \tilde{H}$ that contains part of the boundary of the original surface $\Sigma$ has the same color, without loss of generality red.  It follows that two components of $D \snip \tilde{H}$ that share corresponding portions of the same arc in $\tilde{A}$ have the same color.  Thus, every component of $\Sigma\snip\tilde{H}$ is consistently colored either red or blue; in particular, every component of $\Sigma\snip\tilde{H}$ that contains a boundary cycle of $\Sigma$ is red.

Every face region in the ribbon graph $\Sigma^\square$ is either entirely red or entirely blue, so we can pull the coloring back to the faces of $\Sigma$.  \jnote{Thus $H$ is the boundary of the blue faces.}  The identity $x(H\oplus H') = x(H) \oplus x(H')$ now implies that if two even subgraphs have equal crossing parity vectors, they must be homologous.
