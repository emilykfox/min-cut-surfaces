...
%\anote{It seems that this definition of genus only works for surfaces with no boundary.}\jnote{No, it's correct even for surfaces with boundary.}
 Up to homeomorphism,
<<<<<<< HEAD
there is exactly one orientable surface and one non-orientable surface with any genus $g > 0$ and any number of
...
>>>>>>> 3233c9f5165de2bd20436839c472cfbfe09f3078
...
\enote{From reviewer: Your definition of an embedding is intuitive, but not formally correct (the relative interiors of paths must avoid the images of vertices; there is a condition on the endpoints of the paths). I think it is fine to skip the details, provided you indicate that this is an informal definition.}
=======
An \EMPH{embedding} of an undirected graph $G=(V,E)$ on a surface $\Sigma$ maps vertices to distinct points and edges to simple, interior-disjoint paths %jeffe added:
...
Similarly, an arc or cycle~$\alpha$ and another arc or cycle $\beta$ are \EMPH{non-crossing} if they have disjoint perturbations within the ribbon graph; otherwise, we say that $\alpha$ \EMPH{crosses}~$\beta$.  \note{Ref 2 line 295: More generally, we say that a collection of arcs and/or cycles is \EMPH{non-crossing} if there is a ribbon perturbation of the entire collection in which all perturbed curves are disjoint.}
