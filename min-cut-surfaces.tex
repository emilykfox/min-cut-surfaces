...
%\usepackage{berasans,beramono} %-- bold too heavy
\usepackage[semibold]{sourcesanspro}
%\usepackage[scale=0.95]{ClearSans}
%\usepackage[scale=0.9]{GoSans,GoMono} -- Too narrow
...
\item Notation: $\snip$ vs $\setminus$ vs $\setminus\!\!\setminus$ vs $\fatbslash$
\end{itemize}
\end{TODO}
...
\note{Status: Incomplete; existing parts are okay but may need modernization}
...
\note{Status: Inconsistent -- Revisit after the rest of the paper is finished}
...
\caption{A cellularly embedded graph \emph{G}$G$ (solid lines) on a pair of pants (genus 0 with 3 boundaries), and its dual graph $G^*$ (dashed lines).  Dual boundary vertices are indicated by squares.}
\label{fig:duality}
\end{figure}
...
A continuous map~$\pi \colon \Sigma' \to \Sigma$ between two surfaces is called a
...
\begin{TODO}
\begin{itemize}\cramped
\item Define “$\Z_2$-minimal”
\item Consistent notation!!!!!!
\item Recall $\oplus$ and $\beta$
\end{itemize}
\end{TODO}
...
By running this algorithm $2^\beta$ times, we can compute the shortest directed cycle in $\Sigma$ in \emph{every} $\Z_2$-homology class, in $O(8^\beta \beta^2\, n\log n)$ time.  In particular, we can compute the shortest directed cycle in~$\Sigma$ that has nontrivial $\Z_2$-homology.  When the original surface has no boundary, this is just the shortest \emph{non-separating} cycle in $\Sigma$.  Cabello \etal~\cite{ccl-fsncd-10} described an algorithm to compute the shortest non-separating cycle in any surface-embedded directed graph in $O(g^{1/2} n^{3/2}\log n)$ time; our algorithm is faster whenever $g \le (\lg n)/13$.
...
Finally, we describe our algorithm to compute \emph{global} minimum cuts in surface-embedded graphs, where no source and target vertices are specified in advance.  As in previous sections, it is convenient to work in the dual graph.  We cannot apply Lemma \ref{lem:cut-duality} directly, but the following lemma similarly characterizes global minimum cuts in any surface graph in terms of homology in the dual graph.  A \EMPH{separating subgraph} is any non-empty boundary subgraph, or equivalently, the boundary of the union of a \emph{proper, non-empty} set of faces.

\note{Unlike earlier sections, we need to assume the underlying surface does \textul{not} have boundary.}
...
Let $H$ be any minimum-weight separating subgraph.  If $H$ is connected and has a contractible Euler tour, then $H$ must be a \emph{simple} contractible cycle  \note{because the shortest contractible cycle is simple, by the 3-path condition}.  Otherwise, every cycle decomposition of $H$ consists of one or more cycles, each of which is non-contractible.  See Figure~\ref{fig:global_cases}.
...
In Section \ref{sec:global_contractible}, we describe an algorithm that computes a minimum-weight separating subgraph \emph{if} any minimum-weight separating subgraph is a simple contractible cycle.  Similarly, in Section \ref{sec:global_non-contractible}, we describe an algorithm that computes a minimum-weight separating subgraph \emph{if} any minimum-weight separating subgraph can be decomposed into non-contractible cycles.  In both cases, if no minimum-weight separating subgraph satisfies the corresponding condition, the algorithm still returns a boundary subgraph, but this subgraph could be empty or have large weight.  By running both subroutines and returning the best result, we are guaranteed to find a minimum-weight separating subgraph, no matter which category it falls into.
...
Notation issues:
\begin{itemize}\cramped
\item No scissors; $\snip$ for “snip” and $\Gsnip$ for $G$-after-cutting are  macros, so we can revert if Amir or Kyle object.
\item Are $\alpha$ and $\gamma$ the right notation for a simple cycle and its projection, in Lemma 7.4?
\item Define $e = f|g$ in the prelims.
\item Is $P$ the greedy system of arcs?  What if the surface has no boundary?
\item $\gamma_1$ and $\gamma_2$ for simple cycles is OK.
\item Later: Don’t use $\sigma$ for minimum-weight separating subgraph when it isn’t a cycle.
\end{itemize}
\end{TODO}

We begin by describing an algorithm to handle the case where some minimum weight separating subgraph is a simple contractible cycle.
...
\note{Define $\snip$ in the prelims.}
...
\item
\textbf{Lemma 3.4:} Cut $\Sigma\snip\ell$ along the \emph{union} of these greedy arcs to obtain a combinatorial annulus~$A$.  This takes $O(n)$ time.  The union of the arcs is a greedy cut graph; we don’t have to compute the individual arcs (but we do need them for the correctness proof).
...
Next we consider the case where every minimum-weight separating subgraph consists of noncontractible cycles.  The following lemma, which is the main result of this section, might be of independent interest. \note{Wishy washy.  If we really think this may have further applications, say why.} 

\begin{TODO}
\noindent
Subtleties we may want to make more explicit:
\begin{itemize}\cramped
\item
If $\Sigma\setminus X$ has three components $A$, $B$, and $C$, then the boundary of $A\cup B$ has smaller weight than $X$.  Thus, $\Sigma \setminus \minSS$ has exactly two components.
\item
The proof assumes every edge has two distinct sides; otherwise refine $G$ with infinite-weight edges.
\end{itemize}
\end{TODO}
...
\begin{proof}
If~$H$ lies in the closure of the component of~$\Sigma \setminus \minSS$ not containing $f$, then we are done, so assume otherwise.  See Figure \ref{fig:global_nonsep-vs-shortsep}.


\begin{figure}[ht]
\centering
\includegraphics[height=1.25in]{Fig/nonsep-vs-shortsep-2}
\caption{The setting of Lemma~\ref{lem:global_split-nocross}. A~$\Z_2$-minimal even subgraph~$H$ is separated from face~$f$ by a minimum weight separating subgraph~$\minSS'$. \textbf{Yes, I know this is too crowded.}}
...
For any subgraph~$A$ of $G$, let $w(A)$ denote the sum of the weights of the edges of $A$.\note{Define this earlier.}  Because both~$\minSS'$ and $\minSS$ are null-homologous, the even subgraph $H' = H \oplus \minSS' \oplus \minSS$ is homologous to $H$, and therefore to $\gamma$.  we immediately have $w(H') \ge w(H)$, because $H$ is $\Z_2$-minimal.  Similarly, because $\minSS$ is a minimum-weight separating subgraph, we immediately have $w(\minSS') \ge w(\minSS)$.
...
We conclude that $\minSS’$ is also a minimum-weight seprating subgraph, completing the proof of the lemma.
\end{proof}
...
\begin{lemma}
  ...
  There exists a~$g^{O(g)} n \log \log n$ time algorithm that computes a minimum weight separating subgraph if every minimum weight separating subgraph can be decomposed into non-simple contractible cycles. If not, the algorithm either returns some separating subgraph (that may not be minimum weight) or nothing.
\end{lemma}
\begin{proof}
Our algorithm begins by picking an arbitrary face~$f$ of~$G$. Let~$\minSS$ be an arbitrary minimum weight separating subgraph.
We argue that there exists a non-separating closed walk~$\gamma$ in~$G$ that lies in the closure of the opposite component of~$f$ in~$\Sigma \setminus \minSS$.

By assumption,~$\minSS$ can be decomposed into simple cycles, each of which is non-contractible. Suppose~$\minSS$ consists of more than one cycle. None of the cycles are null-homologous, because we could remove a single null-homologous cycle from~$\minSS$ to lower its cost without changing it homology class.
In this case,~$\gamma$ is simply one of the cycles in~$\minSS$'s decomposition. Now, assume~$\minSS$ consists of a single simple cycle. Cycle~$\minSS$ is not contractible by assumption, so neither component of $\Sigma \setminus \minSS$ is planar. The closure of the component opposite~$f$ has non-zero genus and therefore contains a non-separating closed walk in~$\Sigma$.

Let~$H$ be a shortest even subgraph homologous to~$\gamma$.
By Lemma~\ref{lem:global_split-nocross}, we may assume without loss of generality that~$H$ lies in the closure of the opposite component of~$f$ in~$\Sigma \setminus \minSS$. Assume for now that our algorithm knows~$H$. We will remove this assumption later in the proof.

Our algorithm picks an arbitrary edge~$e = h_1 | h_2$ of~$H$. At least one of~$h_1$ and~$h_2$ lies in the closure of the opposite component of~$f$ in~$\Sigma \setminus \minSS$. Our algorithm computes minimum weight subgraphs separating~$h_1$ from~$f$ and~$h_2$ from~$f$ using the minimum $(s,t)$-cut algorithm of Section~\ref{sec:crossing}. It then returns the cheaper of these two subgraphs, which weighs no more than~$\minSS$.
