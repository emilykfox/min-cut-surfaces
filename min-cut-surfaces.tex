...
As mentioned in Section~\ref{sec:characterizing_crossings}, we cannot consistently define when a shortest path crosses an even subgraph.  We avoid this issue by considering crossings between an entire arc in the greedy system and an arbitrary ribbon perturbation of an arbitrary cycle decomposition of the even subgraph.  We emphasize that different cycle decompositions and different ribbon decompositions of the same even subgraph can have different numbers of crossings with the same arc.  The following crossing bound applies to \emph{every} ribbon decomposition of \emph{every} cycle decomposition.
...
There is a $\Z_2$-minimal even subgraph $H'$ homologous to $H$ such that for \jnote{any ribbon perturbation of} any cycle decomposition
