...
\note{Erin: No mention of Betti number, but we do use it later.  Add here.  Homollogy classes define a vector space over $\Z_2$ whose rank is called the (first) eBetti number of the surface.}

We define a \EMPH{cycle decomposition} of an even subgraph~$H$ to be a set of edge-disjoint, non-crossing cycles whose union is $H$.
\begin{lemma}
...
\note{Undirected everywhere by default; only the homology cover algorithms works for directed graphs; explain diffs there.}
...
\item Finally, $X$ contains the remaining edges $E \setminus (\partial\!G \cup F \cup C^*)$
\end{itemize}

\note{Jeff: We’re overloading the variable $C$ a lot.  I propose $C =$ cotree, $L =$ leftover edges in tree-coforest or forest-cotree decomposition, $X$ or $K =$ cut}
...
\note{Revisit this.  bold-emphasize greedy system of arcs when it’s actually defined.}
...
Let~$G$ be undirected \note{All graphs in this paper are undirected}, and let $H$ be an even subgraph of $G$.  In this section, we describe an
algorithm to compute the minimum-weight even subgraph homologous with
...
\note{Erin: This lemma is cited several times in 4 and 5, but does not appear to exist.  I assume we mean what is currently lemma 2.1?  If so, need to search and replace.  Jeff: The correct label is\texttt{\textbackslash label{lem:cut-duality}}.}
...
\note{$P$ is a greedy system of arcs.  We assume boundary wlog, delete a face otherwise.}

We may now describe our algorithm.
We can cut our combinatorial surface $\Sigma\setminus P$ into a $2\beta$-gon, or \emph{abstract polygonal schema}, by cutting along each path in $P$ and replacing each copy of each path in the cut surface with a single edge.  Thus, each path in $P$ corresponds to two edges of this polygon.
...
\note{Move high-level overview to beginning of section?}
