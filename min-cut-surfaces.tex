...
Our algorithms manipulate cycles and arcs in combinatorial surfaces that can share or repeat graph vertices and edges, but that can be perturbed on the surface to avoid self-intersections and other degeneracies.  To accommodate these perturbations, we represent our combinatorial surfaces as \EMPH{ribbon graphs} \cite{}.  (Ribbon graphs are also known as \emph{band decompositions} \cite{} and \emph{fat graphs} \cite{} , and they are closely related to the \emph{graph-encoded map} data structure \cite{} for representing both orientable and non-orientable combinatorial surfaces.)  The ribbon graph $G^\square$ of an embedded graph $G$ is constructed by expanding the vertices of $G$ into disjoint disks called \emph{vertex regions} and the edges of $G$ into disjoint rectangles called \emph{edge ribbons}.  Each face of the original embedding contains exactly one component of $\Sigma\setminus G^\square$.

\jnote{words words words}  Any finite collection $C$ of cycles and arcs in a combinatorial surface can be perturbed into general position within the ribbon graph $G^\square$, so that all (self-)intersections are transverse and occur within vertex regions.  None of the cycles and arcs we consider contain \emph{spurs}—subpaths that traverse an edge followed immediately by its reversal.  Thus, the intersection of the perturbed cycles and arcs and any edge ribbon is a collection of disjoint paths that traverse the ribbon from one end to the other.  We also require minimal (self-)intersection within each vertex region are minimal; that is, each boundary-to-boundary path within a vertex region is simple, and any two such paths intersect at most once.  This requirement allows us to represent a perturbation of $C$ symbolically by recording, for each edge $e$ of $G$, the left-to-right order of the traversals of the ribbon $e^\square$.

\jnote{Perturbed arcs start at vertex-face boundary of vertex region}
\jnote{This is all equivalent to the cross-metric surface model}
\jnote{Weakly simple = has a simple peturbation}

\jnote{Band decompositions/ribbon graphs.  Replace every vertex with a tiny disk and every edge with a narrow ribbon.  This works even if the surface is non-orientable.  Each edge stores a left-to-right ordering of the cycles and arcs traversing the corresponding edge ribbon.  In particular, cycles and walks can only (self-)intersect within vertex regions.  Arcs extend to the boundary of the regions corresponding to their endpoints.  A cycle or arc is weakly simple if it has a simple representation; two cycles, or a cycle and an arc, are weakly disjoint if they have disjoint representations.  This formulation is equivalent to Èric’s cross-metric surface model.}


\rnote{209: While you're at it, you should probably say a few words about data structures for graph embeddings.}\jnote{We don’t need to talk about rotation systems.}
...
Similarly, an arc or cycle~$\alpha$ and another arc or cycle $\beta$ are \EMPH{non-crossing} if some arbitrarily small perturbations of $\alpha$ and $\beta$ are disjoint; otherwise, we say that $\alpha$ \EMPH{crosses}~$\beta$.  \jnote{Add more careful definition of crossing subpaths, following Amir and Jeff’s paper on non-crossing walks.  That formulation works for us only because (1) arcs have both endpoint on boundary, and (2) neither arcs nor cycles have spurs!}
...
definitions.
Fix a cellular embedding of an undirected graph $G$ on a surface with genus $g$ and $b$ boundaries.  A \EMPH{boundary subgraph} is the boundary of the union of a subset of faces of $G$; for example, every separating cycle \modified{in a surface with no boundary} is a boundary subgraph.
...
Boundary subgraphs are also called \EMPH{null-homologous}.  \modified{An even subgraph is $\Z_2$-minimal, if it is the minimum weight subgraph in its homology class.} Any two homotopic cycles are homologous, but homologous cycles are not necessarily homotopic; see Figure \ref{fig:homology}.  Moreover, the homology class of a cycle can contain even subgraphs that are not cycles; see Figure \ref{fig:homology2}.
...
\sout{This cut partitions the vertices of $G$ into two disjoint subsets~$S$ and $T$, respectively containing vertices $s$ and $t$.}  Thus, the dual subgraph $\Cut^*$ partitions the faces of $G^*$ into two disjoint subsets, $S^*$ and $T^*$, respectively containing faces $s^*$ and $t^*$.  In particular, $\Cut^*$ is the boundary of the union of the faces in $S^*$, which implies that $\Cut^*$ is null-homologous in $\Sigma$.  The symmetric difference $\Cut^* \oplus \partial s^*$ is the boundary of the union of $S^* \setminus\set{s^*}$, which is a subset of the faces of $\Sigma'$.  Thus, $\Cut^*\oplus \partial s^*$ is null-homologous in $\Sigma'$.  We conclude that $\Cut^*$ and  $\partial s^*$ are homologous in $\Sigma'$.
...
\\	&= n + f-1 + \abs{L}
\end{align*}
and therefore $\abs{L} = m - n - f + 1 = 2g - b - 1$.  Thus, in both cases, $L$ contains exactly $\beta$ edges.
}
...
A proof of the lemma appears in Borradaile~\etal~\cite[Corollary 3.5]{bcfn-mchbs-17}.
...
\begin{proof}
\jnote{Rewrite exchange argument at the level of arcs, not paths.  Use the earlier lemma that the greedy system of arcs is the \emph{shortest} system of arcs.  So (later) if any of the contracted loops $\ell_i$ in $\Sigma/a_i$ is contractible, then we could shorten $a_i$ to get a shortest system of arcs, contradiction.}
\knote{At the end of the last meeting, we agreed it would be easier to use the following argument:
If you cross an arc too many times, then you need to \emph{touch} one of its shortest paths too many
times.
Now use the original argument for shortest paths.}

For the sake of argument, we slightly modify the edge weights of $G$.
...
weights as well;
however, every \(Z_2\)-minimal even subgraph under the modified weights also has a bounded number of
crossings with each arc of \(A\) as specified by the lemma.
%
