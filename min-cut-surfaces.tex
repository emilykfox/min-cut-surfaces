...
Slicing along any perturbed arc~$\tilde{a}_i$ increases the Euler characteristic of the surface by
1.
For example, if the surface is orientable, then slicing either reduces the number of boundary components by 1, if the endpoints of $\tilde{a}_i$ lie on distinct boundaries, or 
%\enote{Reviewer says: "Since the surface is allowed to be non-orientable (see line 356), you also need to consider crosscaps. Actually, perhaps just removing the sentence would be better."  I'm not following what they object to here.}
cuts along a handle once, if both endpoints lie on the same boundary.
...
\caption{Adding a handle to transform a negative edge into a negative cocycle.  Thick (blue) edges have cost $C$; dashed edges have cost zero. The cost of each red edge is $c(uv)/6$.}
\label{fig:addhandle}
\end{figure}
