...
\note{Status (excluding subsections): Beta}
...
This paper describes the first algorithms to compute minimum cuts in surface-embedded graphs of fixed genus in near-linear time.  Specifically, we describe two algorithms to compute minimum $(s,t)$-cuts in undirected surface graphs, the first in $g^{O(g)} n\log\log n$ time, and the second in $2^{O(g)} n\log n$ time.  We also extend our first algorithm to find \emph{global} minimum cuts in undirected surface graphs in $g^{O(g)} n\log\log n$ time; our second algorithm actually works without modification in directed surface graphs.  For all our algorithms, the input consists of an $n$-vertex graph with arbitrary real edge weights, embedded on an orientable surface of genus $g$.
...
%
%Finally, the first algorithms to find optimal \emph{directed} cycles in \emph{directed} surface graphs were proposed by Cabello, Colin de Verdi\`ere, and Lazarus~\cite{ccl-fsncd-10}.  In the preliminary version of Section \ref{sec:homcover}~\cite{en-mcsnc-11}, we described an algorithm to compute the shortest non-separating cycle in a directed surface graph in $g^{O(g)} n \log n$ time.
%
%
...
Most interesting variants of homology localization are NP-hard.  In particular, results of Chambers \etal~\cite{ccelw-scsih-08} imply that finding the shortest \emph{simple} cycle in a given homology class in a surface graph is NP-hard.  Chen and Freedman \cite{cf-hrhl-10} proved that homology localization in arbitrary simplicial complexes is NP-hard even to approximate.  Very recently, Grochaw and Tucker-Foltz \cite{gt-ctugc-18} proved that homology localization in surface graphs, over a sufficiently large finite coefficient ring, is equivalent to Unique Games; in particular, there is no PTAS for \emph{any} finite coefficient ring unless the Unique Games Conjecture is false.
...
In Section \ref{S:tree-cotree}, we develop two techniques to measure the homology class of a graph.  Both techniques are natural generalizations the tree-cotree decomposition, introduced by Eppstein \cite{e-dgteg-03} for surfaces without boundary.  This generalization is essential for our algorithms, as our dual homology characterization removes the dual faces $s^*$ and $t^*$, leaving a surface with two boundary components.



In Section~\ref{sec:crossing}, we give the first algorithm to compute optimal cycles in a homology class. 
The technique is as follows: we first cut the surface into a disk and argue by exchange arguments that the shortest cycle cannot cross the boundary of the disk very often.  We then enumerate all cycles crossing the boundary that satisfy the bounds, and for each find the shortest graph with that crossing pattern.  Ultimately, this reduces the problem to finding the minimum cut in a planar graph.

In Section~\ref{sec:homcover}, we then give a different algorithm for the same problem which has a better but still exponential dependance on $g$, but a worse dependence on $n$. For this approach, instead of considering the sequence of crossings with the boundary, we instead count the number of crossings with each edge, building a structure called the $\Z_2$-homology cover.  This reduces the problem of finding the shortest cycles in a homology class to a multiple source shortest path problem in this cover.


In Section~\ref{S:NPhard}, we prove that finding a minimum even subgraph in a given homology class in NP-Hard. 
Unlike Chen and Freedman \cite{cf-hrhl-10}, this reduction is done on a 2-manifold, and unlike Chambers \etal~\cite{ccelw-scsih-08}, the subgraph does not need to be a simple cycle.
This reduction implies that the exponential dependence on $g$ in our algorithms is unavoidable.  

Finally, in Section~\ref{sec:global}, we describe an algorithm to compute the global minimum cut in a weighted surface graph.  This approach combines techniques from our earlier algorithms with a planar global minimum cut algorithm .  
...
The input to our first algorithm is an undirected edge-weighted graph~$G$ embedded on an orientable surface of genus~$g$.  \note{We assume shortest paths are unique; see section 2.x for details.}  Given vertices $s$ and $t$, our algorithm computes a minimum-weight $(s,t)$-cut in $g^{O(g)}n\log \log n$ time.
...
Our minimum-cut algorithm is a special case of a more general algorithm to compute a minimum-cost \emph{subgraph} in a given $\Z_2$-homology class.  Even when the homology class is specified by a simple cycle, the output representative may be the union of several cycles; see Figure \ref{fig:homology2}.  For surfaces with genus $g$ and $b$ boundary components, our algorithm runs in $(g+b)^{O(g+b)}n\log\log n$ time.
...
\begin{itemize}\cramped
\item Formal definition of covering space
...
\note{Erin: Change this later if we alter to include non-orientable double cover results, but for now only orientable}
...
Boundary subgraphs are also called \EMPH{null-homologous}.  Any two homotopic cycles are homologous, but homologous cycles are not necessarily homotopic; see Figure \ref{fig:homology}.  Moreover, the homology class of a cycle can contain even subgraphs that are not cycles; see Figure \ref{fig:homology2}.
...
\note{double-check weakly simple}
\end{proof}
...
\note{Don't need formal definition; just cite textbook.}

A continuous map~$\pi \colon \Sigma' \to \Sigma$ between two surfaces is called a \EMPH{covering map} if each point~$x \in \Sigma$ lies in an open neighborhood~$U$ such that (1)~$\pi^{-1}(U)$ is a countable union of disjoint open sets~$U_1 \cup U_2 \cup \cdots$ and (2) for each~$i$, the restriction~$\pi |_{U_i} : U_i \to U$ is a homeomorphism. If there is a covering map~$\pi$ from~$\Sigma'$ to~$\Sigma$, we call~$\Sigma'$ a \EMPH{covering space} of~$\Sigma$.  The \EMPH{universal cover}~$\tilde{\Sigma}$ is the unique simply-connected covering space of~$\Sigma$ (up to homeomorphism).  The universal cover is so named because it covers every path-connected covering space of~$\Sigma$.

For any path~$p : [0,1] \to \Sigma$ such that~$\pi(x') = p(0)$ for some point~$x' \in \Sigma'$, there is a unique path~$p'$ in~$\Sigma'$, called a \EMPH{lift} of~$p$, such that~$p'(0) = x'$ and~$\pi \circ p' = p$.  We also say that~$p$ \EMPH{lifts} to~$p'$. Conversely, for any path~$p'$ in~$\Sigma'$, the path~$\pi \circ p'$ is called a \EMPH{projection} of~$p'$.

%We define a lift of a cycle~$\gamma \colon S^1 \to \Sigma$ to be the infinite
%path~$\gamma' \to \R \to \Sigma'$ such that
...
\note{Status: Beta -- reread from scratch after rest of paper done}

Throughout the paper, we fix an \EMPH{undirected} directed graph $G=(V,E)$, a non-negative weight function $w\colon E\to \Real$, and a cellular embedding of $G$ on a surface $\Sigma$ of genus $g$ with $b$ boundary cycles.
...
Finally, for each edge $e$ in $G$, we define its signature \EMPH{$[e]$} to be the $\beta$-bit vector whose $i$th bit is equal to~$1$ if and only if $e$ crosses $\dualarc_i$ (that is, if $\dualarc_i$ traverses the dual edge $e^*$) exactly once. \note{Not “an odd number of times”?} The signature $[H]$ of an even subgraph $H$ is the bitwise exclusive-or of the signatures of its edges.  Similarly, the signature~$[\cycle]$ of a cycle $\cycle$ is the bitwise exclusive-or of the signatures of the edges that $\cycle$ traverses an odd number of times.
...
\note{Merge with previous paragraph}
...
\note{Actually this last result works for directed graphs as well.  We really don’t need to say much here.}
...
%\note{kylejfox: There's a commented out note about how we can further reduce to $3^g$ instances of minimum $(s,t)$-cut in a graph of genus $O(g)$. Do we want to put that back in?}
...
As in previous sections, it is convenient to work in the dual graph.  We cannot apply Lemma \ref{lem:cut-duality} directly, but the following lemma similarly characterizes global minimum cuts in any surface graph in terms of homology in the dual graph.  A \EMPH{separating subgraph} is any non-empty boundary subgraph, or equivalently, the boundary of the union of a \emph{proper, non-empty} set of faces.  Unlike previous sections, we assume throughout this section that the underlying surface does \emph{not} have boundary.

\note{We do not assume generic edge weights in this section; in particular, we explicitly allow for multiple minimum-weight separating subgraphs, as well as multiple minimum-weight graphs in each homology class.}
...
%
%To simplify notation, we shift our focus from the original primal graph to its dual.
%\note{Check consistency.}
...
%Let $\minSS$ be any minimum-weight separating subgraph.  If $\minSS$ is connected and has a contractible Euler tour, then $\minSS$ must be a \emph{simple} contractible cycle  \note{because the shortest contractible cycle is simple, by the 3-path condition}.  Otherwise, every cycle decomposition of $\minSS$ consists of one or more cycles, each of which is non-contractible.

Let $\minSS$ be a minimum-weight separating subgraph.  $\Sigma \setminus \minSS$ has exactly two components; otherwise, the boundary of any single component of $\Sigma \setminus \minSS$ a smaller separating subgraph.  \note{This is new:} Abusing terminology slightly, call a separating subgraph $X$ \EMPH{contractible} if either component of $\Sigma \setminus \minSS$ is a disk, and \EMPH{non-contractible} otherwise.  If $\minSS$ is contractible, then $\minSS$ is actually the shortest (weakly) simple contractible cycle in $G$; otherwise, $\minSS$ can be decomposed into one or more simple cycles, each of which is non-contractible.  See Figure~\ref{fig:global_cases}.

%
%\begin{TODO}
%\noindent \textbf{I think this case division is cleaner:}
%
%Let $\minSS$ be a minimum-weight separating subgraph.  $\Sigma \setminus \minSS$ has exactly two components; otherwise, the boundary of any single component of $\Sigma \setminus \minSS$ a smaller separating subgraph.  Abusing terminology slightly, call a separating subgraph $X$ \EMPH{contractible} if either component of $\Sigma \setminus \minSS$ is a disk, and \EMPH{non-contractible} otherwise.
%
%\end{TODO}
...
%\note{Yes, really. Cabello \etal\ reduce finding the shortest contractible closed walk that encloses at least one face to finding a global minimum cut in a planar graph $O(n\log n)$ time \cite{cdem-fotc-10}.  At the time, the fastest planar mincut algorithm ran in $O(n \log^2 n)$ time~\cite{cfn-dnlta-04}, but the more recent algorithm of \L\c{a}cki and Sankowski runs in $O(n \log \log n)$  time~\cite{ls-mcsc-11}.  However, as we (Kyle) already point(s) out, the resulting closed walk may self-overlap everywhere, making the resulting graph in $G$ empty.}
...
%
\footnote{Cabello \etal\ reduce this problem to finding a global minimum cut in a planar graph in $O(n \log n)$ time~\cite{cdem-fotc-10}.  They used the planar minimum-cut algorithm of Chalermsook \etal~\cite{cfn-dnlta-04} to derive a running time of $O(n\log^2 n)$; the \L\c{a}cki and Sankowski runs in $O(n \log \log n)$  time~\cite{ls-mcsc-11}.}
%
...
\note{This lemma needs work!!!  Better to write: Any bounding closed walk in $\Gsnip$ projects to a bounding closed walk in $G$.}

\begin{lemma}
\label{lem:global_null-homologous-projections}
Let~$\alpha$ be an arbitrary simple cycle or arc in~$G$. Let
${\Sigmasnip = \Sigma \snip \alpha}$ and let~$\Gsnip = G \snip \alpha$. Any boundary subgraph~$\gammasnip$ in~$\Gsnip$ projects  to a boundary subgraph in~$G$.
\end{lemma}

\begin{proof}
Let~$\gammasnip$ be an arbitrary boundary subgraph in~$\Gsnip$ and let~$\gamma$ be its projection in~$G$.
Subgraph~$\gamma$ bounds a subset of faces~$\Fsnip$ in~$\Gsnip$. 
Let~$F$ be the projection of~$\Fsnip$ into~$G$.
We will argue that~$\gamma$ is the boundary of~$F$, proving the lemma.

Consider any edge $e$ on the boundary of $F$, which separates two faces $f^+\in F$ and $f^-\not\in F$ in $G$.  Then in $\Gsnip$, either $e$ still separates faces $f^+$ and $f^-$, or $e$ separates one of those two faces from the boundary of $\Gsnip$.

Consider any edge~$e = f | g$ \note{define} on the boundary of~$F$. If~$f$ and~$g$ still lie adjacent along~$e$ in~$\Gsnip$, then~$e$ bounds~$\Fsnip$ and appears in~$\gamma$. If~$e$ separates a face~$f$ from the boundary of~$\Gsnip$, then~$e$ still appears in~$\gamma$.
...
The next lemma presents an algorithm to compute a tight non-separating cycle; this is a modification of an algorithm by  Cabello \etal~\cite[Theorem 5.4]{cdem-fotc-10}, where they give an algorithm that runs in $O(n \log n)$ time for the same problem.  

\begin{lemma}
\label{lem:tightcycle}
There exists an $O(n \log \log)$ algorithm that computes a tight non-separating cycle in a surface without boundary.
\end{lemma}
...
%There exists a~$g^{O(g)} n \log \log n$ time algorithm that computes a minimum weight separating subgraph if any such subgraph is a simple contractible cycle. If not, the algorithm either returns some separating subgraph (that may not be minimum weight) or nothing.
%\end{lemma}
\begin{lemma}
\label{lem:contractible-alg}
There exists a~$O(n \log \log n)$ time algorithm that computes a minimum weight separating subgraph if any such subgraph is a simple contractible cycle. If not, the algorithm either returns some separating subgraph (that may not be minimum weight) or nothing.
\end{lemma}


%\begin{TODO}
%\noindent
%\textbf{\boldmath I think we can get rid of the $g^{O(g)}$ factor in the running time.}  Cabello \etal~find a shortest enclosing cycle in a surface without boundary~\cite[Theorem 5.4]{cdem-fotc-10} as follows.  (Numbered lemmas and theorems are from Cabello \etal~\cite{cdem-fotc-10}.)
%\smallskip
%\begin{itemize}
%\item
%\textbf{Theorem 4.3:} Compute a tight non-separating cycle $\alpha$ in $G$.  This takes Cabello \etal~$O(n\log n)$ time, but $O(n\log\log n)$ is possible:
%
%\begin{itemize}\cramped
%\item
%Compute the shortest non-separating loop $\ell$ through any vertex.  This takes $O(n\log n)$ time via Erickson--Har-Peled (or Erickson-Whittlesey), but the running time can be improved to $O(n)$ via Henzinger \etal~\cite{hkrs-fspap-97} because we assume $g = o(\log n)$.
%
%\item
%\textbf{Lemma 4.1:} Compute the shortest cycle $\alpha$ that is homotopic to $\ell$ in $\Sigma\snip\ell$.
%
%\begin{itemize}\cramped
%\item
%Let $\Sigma'$ be the combinatorial surface obtained by gluing a disk over one boundary of $\Sigma\snip\ell$.
%
%\item 
%\textbf{Lemma 3.4:} (Implicitly) compute a greedy system of (tight) arcs in $\Sigma'$.  This takes $O(n\log n)$ time via Erickson-Whittlesey, but the running time can be improved to $O(n)$ via Henzinger \etal~\cite{hkrs-fspap-97} because we assume $g = o(\log n)$.
%
%\item
%\textbf{Lemma 3.4:} Cut $\Sigma\snip\ell$ along the \emph{union} of these greedy arcs to obtain a combinatorial annulus~$A$.  This takes $O(n)$ time.  (Kutz~\cite{k-csnco-06} did the same thing earlier.)  The union of the arcs is a greedy cut graph; we don't have to compute the individual arcs (but we do need them for the correctness proof).
%
%\item
%Let $\alpha$ be the shortest generating cycle in $A$, computing via planar minimum $(s,t)$-cut.  Cabello \etal\ used Frederickson's $O(n\log n)$ time algorithms, but  Italiano \etal~improve the time bound to $O(n\log \log n)$~\cite{insw-iamcmf-11}.
%
%\end{itemize}
%
%\end{itemize}
%
%\item
%Cut the surface $G\snip \alpha$ along a greedy cut graph, again in $O(n)$ time.
%
%\item
%Finally, compute a minimum cut in the dual of $\Gsnip$.  When that paper was published, the best min-cut algorithm ran in $O(n\log^2 n)$ time~\cite{cfn-dnlta-04}, but this is now $O(n\log\log n)$ \cite{ls-mcsc-11}.
%\end{itemize}
%\smallskip\noindent
%The \text{only} significant differences in our algorithm are that (1) we compute the \emph{shortest} non-separating cycle, and (2) we insist that the enclosing cycle avoid at least one edge on the boundary of $\Gsnip$.  Our Lemma~7.2 implies (1) is unnecessary and enforcing (2) only requires calling the planar min-cut algorithm twice instead of once.
%\end{TODO}
...
%\note{Alternatively, we can refine $G$ with $O(n)$ infinite-weight edges, so that at least one face (or even \emph{every} face) has simple contractible boundary.}  
...
By definition,~$\minSS'$ is a boundary subgraph.  By assumption, $H$ contains at least one edge in the component of $\Sigma\setminus \minSS$ containing $f$; this edge is on the boundary of two far faces \note{because facewidth $>1$}.  On the other hand, each edge of $\gamma$ has a far face on at most one side.  Thus, there is at least one far black face~$f'$.  There is also at least one near face, because $\minSS$ is separating.  We conclude that $\minSS'$ is non-empty.  No edge of~$H$ has black faces on both sides, so~$H$ lies in the closure of the  component of~$\Sigma \setminus \minSS'$ that does not contain $f$.

It remains to argue that~$\minSS'$ is a minimum-weight separating subgraph of $G$.

For any subgraph~$A$ of $G$, let $w(A)$ denote the sum of the weights of the edges of $A$.\note{Define $w(A)$ earlier.}  Because both~$\minSS'$ and $\minSS$ are null-homologous, the even subgraph $H' = H \oplus \minSS' \oplus \minSS$ is homologous to $H$, and therefore to $\gamma$.  we immediately have $w(H') \ge w(H)$, because $H$ is $\Z_2$-minimal.  Similarly, because $\minSS$ is a minimum-weight separating subgraph, we immediately have $w(\minSS') \ge w(\minSS)$.
...
We conclude that $\minSS'$ is also a minimum-weight separating subgraph, completing the proof of the lemma.
\end{proof}
...
\begin{proof}
\note{Rewritten; please double-check!}
...
We would also like to thank the anonymous reviewers of our earlier extended abstracts \cite{cen-mcshc-09, en-mcsnc-11, efn-gmcse-12} for many helpful comments and suggestions.
