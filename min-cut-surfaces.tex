...
\note{Still pending:
\begin{itemize}
\item Discussion of new results in Section 1.2
\item Non-orientable???  Directed???
\item Cosmetics?
\end{itemize}}
...
n\log n)$ time.  Borradaile \etal~\cite{bkmnw-msmsm-17} described an algorithm to
...
\enote{Make sure we add something about:
\begin{itemize}
\item Talk about ribbon graphs (since not in any of the earlier preprints)
\item Talk about tons of work to deal with perturbations and make intuitively disjoint things actually disjoint
\item Mention $\Z_2$ homology cover being simpler, but having the $\log n$ instead of the $\log \log n$ dependence.
\end{itemize}
}
...
\emph{greedy} system of arcs in the input graph $G$, we instead count the \emph{number} of crossings with each arc in an
\emph{arbitrary} system of arcs in the \emph{dual} graph $G^*$.  The resulting vector of crossing numbers for an even subgraph (a subgraph in which every vertex has even degree)
...
Throughout the paper, we fix an \EMPH{undirected} graph $G=(V,E)$, a positive weight function $w\colon E\to \Real$, and a cellular embedding of $G$ on a surface $\Sigma$ of genus $g$ with $b>0$ boundary cycles.
...
Let $G$ be a graph with non-negatively weighted edges, cellularly embedded on a surface of
genus $g$ (possibly with boundary), and let $f$ be an arbitrary face of~$G$.
