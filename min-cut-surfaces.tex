...
Next we consider the case where every minimum-weight separating subgraph can be decomposed into simple cycles, each of which is non-contractible.  At a high level, our algorithm computes a set $P$ of pairs of faces, such that some minimum-weight separating subgraph of $G$ separates at least one pair of faces in $P$.  (Equivalently, $P$ is a set of pairs of vertices of $G^*$, such that the global minimum cut in $G^*$ is an $(s,t)$-cut for some pair $\set{s,t}\in P$.)  Then for each pair in $P$, we compute a minimum-weight subgraph separating that pair using one of our earlier algorithms.
...
\begin{lemma}
  ...
  There exists a~$g^{O(g)} n \log \log n$ time algorithm that computes a minimum weight separating subgraph if every minimum weight separating subgraph can be decomposed into simple non-contractible cycles. If not, the algorithm either returns some separating subgraph (that may not be minimum weight) or nothing.
\end{lemma}
...
\item
For each non-trivial homology class $h$:
\begin{itemize}
\item
Compute the minimum-weight subgraph $H_h$ in class $h$.
\item
Fix an arbitrary edge $e$ of $H_h$.
\item
If $\emph{left}(e)\ne f$, compute the minimum-weight subgraph separating $\emph{left}(e)$ from $f$.

\item
If $\emph{right}(e)\ne f$, compute the minimum-weight subgraph separating $\emph{right}(e)$ from $f$.

\end{itemize}

\item
Return the smallest separating subgraph found, if any.
...
\begin{itemize}
\item Suppose~$\minSS$ consists of more than one cycle. None of the cycles is null-homologous, because we could remove a single null-homologous cycle from~$\minSS$ to lower its cost without changing it homology class.
