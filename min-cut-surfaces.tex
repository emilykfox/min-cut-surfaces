...
\note{dig out citations}
To simplify our presentation, as in many earlier papers \cite{blah,blah,blah}, we assume that edge weights are \emph{generic}; in particular, we assume that there is a \emph{unique} shortest path between any two vertices of the input graph.  This assumption can be enforced if necessary by symbolic perturbation, with a small penalty in the running time of our algorithms.  
Cabello \etal~\cite{cce-msspe-13} describe two such perturbation schemes.  The first is a randomized scheme based on the Isolation Lemma of Mulmuley \etal~\cite{??}, which increases the running times of our algorithms at most a constant factor \emph{with high probability}.  The second is an efficient implementation of lexicographic perturbation \cite{??} that increases the worst-case running times of our algorithms by a factor of $O(\log n)$.  More recently, Fox \etal \cite{holiest} describe a perturbation scheme based on integer homology that increases the worst-case running times by a factor of $O(g)$.
...
\note{Status: Jeff and Erin are here.  Normalize notation/terminology}
