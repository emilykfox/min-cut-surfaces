...
\note{add combinatorial surface here?}
...
\note{double check boundary stuff is clear}
...
\note{All the homology $Z_2$ stuff should be here now, but double check with list at beginning}
...
We use the cutting operation ($\snip$) along tight cycles and arcs in~$G$.  The following lemma implies it is safe for our algorithm to find minimum-weight separating subgraphs in snipped copies of~$\Sigma$.
