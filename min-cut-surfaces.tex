...
Similarly, an arc or cycle~$\alpha$ and another arc or cycle $\beta$ are \EMPH{non-crossing} if they have disjoint perturbations within the ribbon graph; otherwise, we say that $\alpha$ \EMPH{crosses}~$\beta$.  \jnote{Ref 2 line 295: More generally, we say that a collection of arcs and/or cycles is \EMPH{non-crossing} if there is a ribbon perturbation of the entire collection in which all perturbed curves are disjoint.}
