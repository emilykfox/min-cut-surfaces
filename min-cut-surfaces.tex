...
\begin{TODO}
Compare our algorithm with \cite[Theorem 5.4]{cdem-fotc-10}.  Is the only difference that we’re insisting that the cycle avoid at least one copy of one edge of one arc?  If so, this phase already runs in $O(n \log n)$ time, with no $g$-dependence.  If not, add text to describe difference.
\end{TODO}
...
\end{figure}
\begin{proof}
Assume that~$H$ lies in the closure of the component of~$\Sigma \setminus \minSS$ not containing $f$, then we are done, so assume otherwise.
...
By definition,~$\minSS'$ is a boundary subgraph.  By assumption, $H$ has edges that are incident to two far faces, but $\gamma$ does not; thus, there is at least one far black face~$f$.  Since there is also at least one near face, $\minSS'$ is non-empty. 
No edge of~$H$ lies between two far black faces so~$H$ lies in the closure of the opposite component of~$f$ in~$\Sigma \setminus \minSS'$. We claim~$\minSS'$ is a minimum weight even subgraph.

For the sake of contradiction, suppose~$\minSS'$ is not a minimum weight even subgraph.
Because both~$\minSS'$ and $\minSS$ are null-homologous, the even subgraph $H' = H \oplus \minSS' \oplus \minSS$ is homologous to $H$, and therefore to $\gamma$.

For any subgraph~$H$ of $G$, let $w(H)$ denote the sum of the weights of the edges of $H$.  We now prove that $w(\minSS') + w(H') \leq w(H) + w(\minSS)$ by bounding the contribution of each edge $e \in E(G)$ to both sides of the inequality.  Note that both $\minSS'$ and $H'$ are subgraphs of $\minSS\cup H$; moreover, $\minSS' \oplus H' = \minSS \oplus H$.  There are three cases to consider.
\begin{itemize}
\item
If $e \not\in H \cup \minSS$, then $e$ contributes $0$ to both sides of the inequality.
\item
...
On the other hand, because $\minSS'$ is not a minimum weight separating subgraph, we must have $w(\minSS') > w(\minSS)$. 
It immediately follows that $w(H') < w(H)$, which contradicts the minimality of $H$.
\end{proof}
