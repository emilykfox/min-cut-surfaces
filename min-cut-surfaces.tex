...
Fix a system of arcs $a_1, a_2, \dots, a_\beta$.  In this section we define the \EMPH{crossing parity vector} of any even subgraph $H$, with respect to the fixed system of arcs, and prove that this vector characterizes the homology class of $H$.  Intuitively, the crossing parity vector is a vector of $\beta$ bits, whose $i$th bit is equal to $1$ if and only if $H$ crosses arc $a_i$ an odd number of times.  However, some care is required to ensure that this intuitive notion is actually consistent.  Rather than working directly with $H$, we formally define the crossing parity vector of an arbitrary \emph{ribbon perturbation} of an arbitrary \emph{cycle decomposition} of $H$, and then argue that the resulting bit vector is the same for any cycle decomposition and any ribbon perturbation theoreof.

First, fix a weakly simple cycle $\gamma$ in $G$ and an index $i$.  Let $\tilde{\gamma}$ and $\tilde{a}_i$ be any ribbon perturbation of $\gamma$ and $a_i$.  By definition, all intersections between $\tilde\gamma$ and $\tilde{a}_i$ are transverse crossings, and each vertex region $v^\square$ contains at most one such crossing.  We define the crossing parity $\bar{x}(\gamma)$ to be $1$ if $\tilde\gamma$ intersects $a_i$ an odd number of times, and $0$ otherwise.  \jnote{$\bar{x}(\gamma)$ is independent of the perturbation.}

\jnote{Here’s how I would like to do this, but I don’t know how much detail we need.  Consider two perturbations $\tilde{\gamma}$ and $\tilde{\gamma}’$, both of which intersection $\tilde{a}_i$ transversely.  Because $\tilde{\gamma}$ and $\tilde{\gamma}’$ are homotopic, there is a finite sequence of \emph{homotopy moves} from one perturbation to the other [CITE, FIGURE].  Each homotopy move involving both the moving cycle and $\tilde{a}_i$ preserves crossing parity.}

\medskip\centerline{\jnote{HERE}}\medskip
