...
Fix an index $i$.  For each index $j$, let $x_j$ denote the number of intersection points in $\tilde\sigma_i \cap \tilde\gamma_j$.  Finally, let $x = \sum_j x_j$.  To complete the proof of the lemma, it suffices to prove that $x\le 6g+2b-3$.  (An identical argument bounds the total number of crossings with $\tilde\tau_i$.)
